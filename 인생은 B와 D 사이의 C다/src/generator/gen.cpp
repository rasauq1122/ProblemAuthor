...
vector<pair<int, int>> make_bias(int n) {
    random_device rand;
    mt19937 seed(rand());
...
vector<pair<int, int>> make_octo(int n) {
    random_device rand;
    mt19937 seed(rand());
...
signed main() {
    ...
    for (int case_i = 0 ; case_i < CASE ; case_i++) {
        ...
        n = 111222;
        vector<pair<int, int>> tree = make_octo(n);
        
        edge = vector<vector<int>>(n+1);
        dedge = vector<vector<int>>(n+1);

        memset(dp, -1, sizeof(dp));
        b = gen(seed), d = gen(seed);
